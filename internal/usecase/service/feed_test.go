...
func TestFeedService_CollectPosts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			ExpectedOutput: somePosts,
			ExpectedErr:    nil,

			SetupFeedMock: func(repo *mock_repository.MockFeedRepository, email string, dto *dto.FeedDTO) {
				repo.EXPECT().GetFeedPosts(email, dto).Return(somePosts, nil)
			},

			SetupPostMock: func(repo *mock_repository.MockPostRepository) {
				for ind := range somePosts {
					repo.EXPECT().GetPostSenderInfo(uint(ind)+1).Return(nil, nil, nil).AnyTimes()
					repo.EXPECT().GetPostAttachments(uint(ind)+1).Return(nil, nil).AnyTimes()
				}
...

		{
			...
			ExpectedOutput: somePosts[:2],
			ExpectedErr:    nil,

			SetupFeedMock: func(repo *mock_repository.MockFeedRepository, email string, dto *dto.FeedDTO) {
				repo.EXPECT().GetFeedPosts(email, dto).Return(somePosts[:2], nil)
			},
