	...
	tests := []struct {
	...
	}{
		{
			...
			SetupPostMock: func(repo *mock_repository.MockPostRepository, email string, postID uint) {
				...
				repo.EXPECT().GetPostAttachments(postID).Return(nil, nil)
			},
		},
	...
	tests := []struct {
	...
	}{
		{
			...
			SetupPostMock: func(repo *mock_repository.MockPostRepository, email string, dto *dto.PostsGetByLink) {
				...
				repo.EXPECT().GetPostAttachments(postID).Return(nil, nil).AnyTimes()
			},
		},
...
func TestPostService_CreatePost(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			ExpectedOutput: nil,
			ExpectedErr: &apperror.ServerError{
				UserErr: apperror.BadRequest,
			},
