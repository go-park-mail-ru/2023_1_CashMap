...
//func (storage *MessageStorage) GetMembersByChatID(senderEmail string, chatID uint) (uint, error) {
//	isChatMember, err := storage.isChatMember(senderEmail, chatID)
//	if err != nil {
//		return 0, err
//	}
//
//	if !isChatMember {
//		return 0, errors.New("you are not member of this chat")
//	}
//
//	rows, err := storage.db.Queryx("SELECT profile.link as link FROM ChatMember AS member" +
//		" JOIN Chat as chat ON chat.id = member.chat_id JOIN UserProfile as profile ON profile.id = member.user_id WHERE chat.chat_id = $1")
//
//	if err != nil {
//		return 0, err
//	}
...
func NewMessageStorage(db *sqlx.DB) repository.MessageRepository {
